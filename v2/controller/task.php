...
elseif(empty($_GET)) {
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // if task fails to create due to data types, missing fields or invalid data then send error json
      ...
      $response->setHttpStatusCode(400);
      $response->setSuccess(false);
      $response->addMessage("Failed to create task. ".$ex->getMessage());
      $response->send();
      exit;
    }
    // if error with sql query return a json error
      ...
      $response->setHttpStatusCode(500);
      $response->setSuccess(false);
      $response->addMessage("Failed to insert task into database - check submitted data for errors ".$ex->getMessage());
      $response->send();
      exit;
    }
  }
